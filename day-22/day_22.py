...
def part_2(lines, output_graph=False):
    ...
    num_bricks = len(supporting_list)
    #supported_by = [set(s) for s in supported_by]
    #supporting_list = [set(s) for s in supporting_list]
    # check supported by and supporting list describe reverse of same graph
    for i in range(num_bricks):
        for j in supporting_list[i]:
    ...
    def num_to_fall(brick_to_remove):
        ...
        while len(queue) > 0:
            ...
            if len(supported_by_copy[b]) == 0:
                ...
                for s in supporting_list[b]:
                    if b in supported_by_copy[s]:
